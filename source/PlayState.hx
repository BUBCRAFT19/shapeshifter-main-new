...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'du':
				var bg:BGSprite = new BGSprite(0, 0, '', null, 0, 0, ClientPrefs.globalAntialiasing);
				bg.makeGraphic(FlxG.width, FlxG.height, FlxColor.WHITE);
				add(bg);
			case 'stage': // Default
				var bg:BGSprite = new BGSprite(-600, -200, 'stageback', null, 0.9, 0.9, ClientPrefs.globalAntialiasing);
				add(bg);

				var stageFront:BGSprite = new BGSprite(-650, 600, 'stagefront', null, 0.9, 0.9, ClientPrefs.globalAntialiasing);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
				add(stageFront);
				if (!ClientPrefs.lowQuality)
				{
					var stageLight:BGSprite = new BGSprite(-125, -100, 'stage_light', null, 0.9, 0.9, ClientPrefs.globalAntialiasing);
					stageLight.setGraphicSize(Std.int(stageLight.width * 1.1));
					stageLight.updateHitbox();
					add(stageLight);
					var stageLight:BGSprite = new BGSprite(1225, -100, 'stage_light', null, 0.9, 0.9, ClientPrefs.globalAntialiasing);
					stageLight.setGraphicSize(Std.int(stageLight.width * 1.1));
					stageLight.updateHitbox();
					stageLight.flipX = true;
					add(stageLight);

					var stageCurtains:BGSprite = new BGSprite(-500, -300, 'stagecurtains', null, 1.3, 1.3, ClientPrefs.globalAntialiasing);
	...
	function eventPushed(event:EventNote)
	{
		switch (event.event)
		{
			...
			case 'Dadbattle Spotlight':
				dadbattleBlack = new BGSprite(-800, -400, '', null, 0, 0, ClientPrefs.globalAntialiasing);
				dadbattleBlack.makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), FlxColor.BLACK);
				dadbattleBlack.alpha = 0.25;
				dadbattleBlack.visible = false;
				add(dadbattleBlack);

				dadbattleLight = new BGSprite(400, -400, 'spotlight', null, 1, 1, ClientPrefs.globalAntialiasing);
				...
				var offsetX = 200;
				var smoke:BGSprite = new BGSprite(-1550 + offsetX, 660 + FlxG.random.float(-20, 20), 'smoke', null, 1.2, 1.05, ClientPrefs.globalAntialiasing);
